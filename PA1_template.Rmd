...
ggplot(aes(day, steps, colour = month), data=act) + geom_histogram(stat = "identity") + facet_wrap(~month) + ggtitle(label = "Number of total daily steps")
```
...
ggplot(aes(interval, steps), data = act) + geom_bar(stat = "identity") + facet_grid(day~month) 
```
...
#plot the total number of steps each days for the two months
ggplot(aes(day, steps), data=act.new) + geom_histogram(stat = "identity") + facet_grid(~month, labeller = ) + ggtitle(label = "Number of total daily steps") + labs(x="Month Day", y = "Steps")
```
